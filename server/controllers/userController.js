...
export const registerUser = async (req, res) => {
  try {
    ...
    if (usernameTrimmed.length < 5) {
      return res
        .status(400)
        .json({ message: "Username needs to be at least 5 characters long." });
    }
    ...
    if (!pattern.test(password)) {
      return res.status(400).json({
        message:
          "Password needs to contain at least one letter and one special character.",
      });
    }
    ...
    // save the uid of a new user in a session
